...
{
    [SerializeField]
    private List<GameObject> normalDebuffPrefabs;
    [SerializeField]
    private List<GameObject> bossDebuffPrefabs;
    ...
    protected override Monster Create(MONSTER_DEBUFF _type, Dictionary<int, Stat> Monsterdict)
    {
        ...
        switch (_type)
        {
            case MONSTER_DEBUFF.NORMAL :
                idx = Random.Range(0, normalDebuffPrefabs.Count);
                debuffMonster = Instantiate(this.normalDebuffPrefabs[idx]).GetComponent<DebuffMonster>();
                break;
            case MONSTER_DEBUFF.BOSS :
                idx = Random.Range(0, bossDebuffPrefabs.Count);
                debuffMonster = Instantiate(this.bossDebuffPrefabs[idx]).GetComponent<DebuffMonster>();
                break;          
        }
        //json������ �����ͼ� ������ ���� �����ϱ�
