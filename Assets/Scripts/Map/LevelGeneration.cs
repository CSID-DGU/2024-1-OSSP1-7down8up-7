...
public class LevelGeneration : MonoBehaviour
{
    ...
    void CreateRooms()
    {
        ...
        for (int i = 0; i < numberOfRooms - 1; i++)
        {
            ...
            //finalize position
            // 마지막으로 생성한 방의 타입을 2로 설정
            int roomType = (i == numberOfRooms - 2) ? 2 : 0;
    ...
    void SetRoomDoors()
    {
        for (int x = 0; x < ((gridSizeX * 2)); x++)
        {
            for (int y = 0; y < ((gridSizeY * 2)); y++)
            {
                ...
                {
                    if (rooms[x, y + 1].type == 2)
                    {
                        rooms[x, y].nextToBossRoom = 1;
                    }
                }

                if (x - 1 >= 0 && rooms[x - 1, y] != null)
                {
                    if (rooms[x - 1, y].type == 2)
                    {
                        rooms[x, y].nextToBossRoom = 2;
                    }
                }
